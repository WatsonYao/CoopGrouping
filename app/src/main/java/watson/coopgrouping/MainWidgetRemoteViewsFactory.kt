...
class MainWidgetRemoteViewsFactory(
  ...
  private var regularSchedules: List<Node> = emptyList()
  private var teamSchedules: List<Node> = emptyList()
  private var bigRunSchedules: List<Node> = emptyList()
  private var combinedSchedules: MutableList<Node> = mutableListOf()
    ...
    runBlocking {
      try {
        ...
        if (response.isSuccessful) {
          response.body()?.let {
            regularSchedules = it.data.coopGroupingSchedule.regularSchedules.nodes ?: emptyList()
            teamSchedules = it.data.coopGroupingSchedule.teamContestSchedules.nodes ?: emptyList()
            bigRunSchedules = it.data.coopGroupingSchedule.bigRunSchedules.nodes ?: emptyList()
  ...
  override fun getViewAt(position: Int): RemoteViews {
    ...
    when (item) {
      ...
      is BigRunSchedule -> {
        ...
        title = "Run"
        details = item.setting?.coopStage?.name ?: "Unknown Stage"
        time = "${item.startTime} - ${item.endTime}" // Format
      }
    }
